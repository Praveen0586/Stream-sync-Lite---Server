...
const service = new service_1.FcmService();
class FcmController {
    ...
    async deleteNotification(req, res) {
        try {
            ...
            const [result] = await db_1.default.query(`DELETE FROM notifications WHERE id = ? AND user_id = ?`, [notificationId, userId]);
    ...
    async markAsRead(req, res) {
        try {
            ...
            const [result] = await db_1.default.query(`UPDATE notifications SET is_read = 1 WHERE user_id = ? AND id IN (?)`, [userId, notificationIds]);
